/**
 * ViabilityScan Worker — IBM Watsonx + Cloudflare Edge
...
export default {
  async fetch(request, env, ctx) {
    ...
    if (url.pathname === '/healthz' || url.pathname === '/readyz' || url.pathname === '/api/health') {
      return json({ status: 'ok', service: 'ViabilityScan', version: '2.0.0', driver: env.DRIVER || 'IBM Watsonx Granite', timestamp: new Date().toISOString() });
    }
...
async function handleAuthCallback(request, env) {
  ...
  if (!clientSecret) {
    return Response.redirect('/?error=secret_config', 302);
  }
  ...
  response.headers.set('Set-Cookie', `vs_session=${sessionId}; HttpOnly; Secure; SameSite=Lax; Path=/; Max-Age=86400`);
  return response;
}
  ...
  try {
    ...
    try {
      const resp = await fetch(url, {
        headers: { 'User-Agent': 'ViabilityScan/2.0', 'Accept': 'text/plain,text/html,*/*' },
...
function buildScanPrompt(code, mode) {
  return `You are ViabilityScan. Analyze this code for production viability. ${mode === 'full' ? 'Deep' : 'Standard'} analysis:
...
function normalizeReport(p, code, mode) {
  return {
    tool: 'ViabilityScan', version: '2.0.0', scanned_at: new Date().toISOString(),
